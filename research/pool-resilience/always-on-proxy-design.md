...
5. **[RESOLVED] Default mode at install:** `passthrough` — safe floor. Install **does** start the daemon and wire `ANTHROPIC_BASE_URL` (so crash/reboot protection is active from install), but stays in passthrough (own token, holds no tokens). Token-swap turns on when the user **joins or starts a pool** (`pool join`/`pool start` → `swap`); `pool stop` → back to `passthrough`.
