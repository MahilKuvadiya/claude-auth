...
export function SignIn() {
  ...
  const go = async () => { setBusy(true); try { await signInWithGoogle(); } finally { setBusy(false); } };
    ...
    <div className="app-wash grid h-screen place-items-center p-6">
      <Card className="w-full max-w-[400px] p-8">
        <div className="text-center">
          ...
          <p className="mb-6 mt-1 text-[.9rem] text-muted-foreground">Sign in with your devxlabs account.</p>
        </div>
        <Button variant="outline" size="md" className="w-full" disabled={busy} onClick={go}>
